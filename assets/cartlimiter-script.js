...
async function resetCart() {
  if (window.productLimit?.length && window.initialShopifyCart?.items?.length) {
    ...
    for (item of localcart) {
      ...
      let limit = !!productLimits.find((i) => i[item.variant_id])
        ? productLimits.find((i) => i[item.variant_id])[item.variant_id].qty
        : 99;
...
function getMetafieldLimits() {
  fetch(`${window.location.origin}/cart?view=alternative`).then((res) =>
    res
      .json()
      .then((data) => {
...
function isAddRequest(payloadString, cartItems, productLimit) {
  ...
  if (parsedPayload) {
    ...
    if (parsedPayload.hasOwnProperty("line")) {
      ...
      // item line position means the place in the cart arrays each item has. 
      //Last added and first array position by default
      ...
      if (parsedPayload.quantity > itemLimit) {
        ...
        window.initialShopifyCart.items[cartItemIndex].quantity =
          parsedPayload.quantity;
        //if property quantity sets product to 0, then delete the item from local variable cart
        window.initialShopifyCart.items =
          window.initialShopifyCart.items.filter((i) => i.quantity !== 0);
      }
      ...
      const itemLimit = !!productLimit.find((i) => i[itemId])
        ? productLimit.find((i) => i[itemId])[itemId].qty
        : 99;
      ...
      cartItemIndex !== -1
        ? (window.initialShopifyCart.items[cartItemIndex].quantity += 1)
        : window.initialShopifyCart.items.unshift({
            variant_id: itemId,
            quantity: 1,
            id: itemId,
          });
      ...
      return Promise.resolve({
        ...
        json: () =>
          Promise.resolve({
            ...
            message: "Cart Error",
            description:
              "You cannot add more than the maximum allowed quantity to the cart.",
          }),
      });
    } else {
